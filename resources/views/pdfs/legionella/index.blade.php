...
<div class="row clearfix block-header-row mt-3" style="background-color:#76D292; color:White !important;">
            <div class="col-xs-12 mt-3">
                <h6 class="text-center" >CONTROL DE LEGIONELA</h6>
            </div>
</div>
...
<table class="table">
            ...
            <tbody>
                ...
                <tr style="width=100%">
                <td><b>Ultima revisión: </b>{{\Carbon\Carbon::parse($legionellaControl["last_treatment_date"])->format('d/m/Y')}}</td>
                <td><b>Proxima revisión: </b>{{\Carbon\Carbon::parse($legionellaControl["next_treatment_date"])->format('d/m/Y')}}</td>
                <td colspan="2"><b>Requiere muestra: </b>{{$legionellaControl["sample_required"] ? 'Si':'No'}}</td>
                <td colspan="2"><b>Temperatura del agua: </b>{{(empty($legionellaControl["water_temperature"]) ? 'No aplica' : $legionellaControl["water_temperature"])}}</td>
                </tr>

                <tr style="width=100%">
                <td><b>Nivel de cloro residual: </b>{{(empty($legionellaControl["residual_chlorine_level"]) ? 'No aplica' : $legionellaControl["residual_chlorine_level"])}}</td>
                <td><b>Tecnico que ejecuta: </b>{{$legionellaControl["worker"]["full_name"]}}</td>
                <td colspan="2"><b>Dentro de límites críticos: </b>{{$legionellaControl["within_critical_limits"] ? 'Si':'No'}}</td>
                <td colspan="2"><b>Producto aplicado: </b>{{$legionellaControl["product"]["name"]}}</td>
