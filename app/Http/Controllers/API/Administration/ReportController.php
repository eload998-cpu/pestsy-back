...
class ReportController extends Controller
{
    ...
    public function __construct(Report $file)
    {
        $this->file = $file;
    ...
    public function index(Request $request)
    {
        ...
        $files = $files->whereRaw("LOWER(reports.name) ILIKE '%{$search_value}%'");
        ...
        $data = DB::transaction(function () use ($request) {
            ...
            foreach ($request->files as $key => $value) {
                ...
                foreach ($value as $f) {
                    ...
                    // Location
                    $location = storage_path() . "/app/public/files/reports/{$request->client_id}/{$filename}";
                    // Compress Image
