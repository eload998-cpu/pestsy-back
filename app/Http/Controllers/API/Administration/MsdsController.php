...
class MsdsController extends Controller
{
    ...
    public function __construct(Msds $file)
    {
        $this->file = $file;
    ...
    public function index(Request $request)
    {
        ...
        $files = $files->whereRaw("LOWER(msds.name) ILIKE '%{$search_value}%'");
        ...
        $data = DB::transaction(function () use ($request) {
            ...
            foreach ($request->files as $key => $value) {
                ...
                foreach ($value as $f) {
                    ...
                    $filename = rand() . '_' . $f->getClientOriginalName();

                    $path = Storage::disk('public')->putFileAs('files/Msds', new File($f), $filename);
                    // Location
                    $location = storage_path() . "/app/public/files/Msds/{$filename}";
                    // Compress Image
                    $storage_link = "/storage/files/Msds/{$filename}";
